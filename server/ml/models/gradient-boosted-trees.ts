 ...
 * Fits sequential weak regression trees to negative gradient pseudo-residuals with shrinkage
 */
export class GradientBoostedTreesRegressor {
  public numEstimators: number;
  public learningRate: number;
  public maxDepth: number;
  public basePrediction: number = 0;
  public trees: DecisionTreeRegressor[] = [];
  public featureImportances: number[] = [];

  constructor(numEstimators: number = 35, learningRate: number = 0.08, maxDepth: number = 5) {
  ...
  public fit(trainSet: MLFeatureVector[]): void {
    ...
    // Initial base prediction: mean of target values
    this.basePrediction = y.reduce((a, b) => a + b, 0) / numSamples;
    const currentPredictions = new Array(numSamples).fill(this.basePrediction);

    this.trees = [];
    this.featureImportances = new Array(numFeatures).fill(0);

    for (let m = 0; m < this.numEstimators; m++) {
      // Compute pseudo-residuals: r_i = y_i - F_{m-1}(x_i)
      const residuals: number[] = [];
      for (let i = 0; i < numSamples; i++) {
        residuals.push(y[i] - currentPredictions[i]);
      }

      // Train a weak decision tree on residuals
      const tree = new DecisionTreeRegressor(this.maxDepth, 6, 3, 0.85);
      ...
      // Update current ensemble predictions with shrinkage
      for (let i = 0; i < numSamples; i++) {
        const stepPred = tree.predictRow(X[i]);
        currentPredictions[i] += this.learningRate * stepPred;
      }
    }
  ...
  public predict(features: number[]): number {
    ...
    for (const tree of this.trees) {
      pred += this.learningRate * tree.predictRow(features);
    }
